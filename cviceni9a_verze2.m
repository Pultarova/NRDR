...
function abc
clc;
L1 = 2.3; % delka oblasti ve smeru x1
L2 = 2; % delka oblasti ve smeru x2
N1 = 18; % SUDY pocet intervalu ve smeru x1
N2 = 14; % SUDY pocet intervalu ve smeru x2
...
L_shape = 1; % chceme L-shape nebo ne (1 nebo 0)
Q = 0.6; % jak velky gradient vzhledem k nejvetsimu je pripustny
...
if (L_shape==1)
    ...
    uzel_hra = Najdi_uzel_hra(xx1,xx2,Nuzel,L1,L2,NeumannOP); % najdeme hranicni uzly
end;
...
function z = Najdi_uzel_hra(xx1,xx2,Nuzel,L1,L2,Neumann)
epsi = 1e-8;
if (Neumann>0) cc = 1; else cc = 0; end;
uzel_hra = zeros(Nuzel,1);
for k = 1:Nuzel
    if (abs(xx1(k)-L1)<epsi || abs(xx1(k)-L1*cc)<epsi || abs(xx2(k)-L2)<epsi || abs(xx2(k))<epsi)
        uzel_hra(k) = 1;
    end;
